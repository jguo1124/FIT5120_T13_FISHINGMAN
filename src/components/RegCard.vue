<script setup>
// A compact renderer for regulation cards.
// - items: normalized rows [{ species, zone, rule, _source, ... }]
// - hideNoRestrictions: when true, hide species with *no size limit* (unless NO TAKE)
//   (Min empty/0 AND Max empty/"No Limit"). Change ONE line below to use "strict mode".
...
/** Derive convenience flags for a row */
function deriveFlags(nr) {
  ...
  const noTake = isZero(daily); // bag limit = 0  -> NO TAKE

  // "Strict mode": only hide when *no size* AND *no daily limit*
  // const noAnyRestriction = noSizeLimit && (noDailyLimit || isNilOrEmpty(daily));

  // "Size-only mode" (default here): hide when *no size limit*, unless NO TAKE
...
/* -------------------- filtering -------------------- */
// When toggled, hide "no restriction" species.
// In default "size-only mode" we hide rows with *no size limit*,
// but we keep NO TAKE rows visible (bag=0).
const list = computed(() => {
  if (!props.hideNoRestrictions) return props.items;
  return props.items.filter((nr) => {
    const f = deriveFlags(nr);
    // keep NO TAKE always visible
    if (f.noTake) return true;
    // hide if noAnyRestriction (per mode) is true
    return !f.noAnyRestriction;
  });
});
</script>

<template>
  <ul v-if="list.length" class="reg-list">
    <li v-for="sr in list" :key="sr.species?.code" class="reg-card">
      <div class="reg-head">
        <div class="title">
...
.reg-card {
  border: 1px solid #e5e9f2; border-radius: 12px; padding: 14px;
...
.reg-card:hover { transform: translateY(-2px); box-shadow: 0 2px 6px rgba(15,23,42,.05), 0 12px 28px rgba(15,23,42,.10); border-color: #dde3ee; }

.reg-head { display: flex; justify-content: space-between; align-items: baseline; gap: 10px; flex-wrap: wrap; margin-bottom: 12px; }
.title { display: flex; align-items: center; gap: 8px; }
.pill { background: #f3f7ff; color: #0d9bb5; border: 1px solid #c8d7ff; padding: 4px 8px; font-size: 12px; border-radius: 999px; font-weight: 700; }
